...
export class AlertProcessor extends WorkerHost {
  ...
  async process(
    ...
    this.logger.log(
      `[PROCESSOR] process START - jobId=${job.id}, alertId=${alertId}, userId=${userId}, attempt=${job.attemptsMade + 1}`,
    );

    try {
      // Get alert details
      this.logger.log(
        `[PROCESSOR] Fetching alert details for alertId=${alertId}`,
      );
      ...
      this.logger.log(
        `[PROCESSOR] ALERT FOUND - alertId=${alertId}, query="${alert.query}", region="${alert.region}", maxArticles=${alert.maxArticles}, isActive=${alert.isActive}, lastRunAt=${alert.lastRunAt?.toISOString() || "never"}`,
      );

      if (!alert.isActive) {
        this.logger.log(
          `[PROCESSOR] Alert ${alertId} is INACTIVE, skipping execution`,
        );
      ...
      this.logger.log(
        `[PROCESSOR] Alert execution completed - articlesProcessed=${result.articlesProcessed}`,
      );

      // Update alert after run
      this.logger.log(`[PROCESSOR] Updating alert after run...`);
      await this.alertsService.updateAlertAfterRun(
        alertId,
      ...
      this.logger.log(
        `[PROCESSOR] Completed alertId=${alertId}: ${result.articlesProcessed} articles processed`,
      );
      ...
      this.logger.error(
        `[PROCESSOR] Full error: ${error instanceof Error ? error.stack : String(error)}`,
      );
  ...
  private async executeAlert(
    ...
    this.logger.log(
      `[PROCESSOR] executeAlert START - alertId=${alertId}, query="${alert.query}", region="${alert.region}"`,
    );
    ...
    this.logger.log(
      `[PROCESSOR] Using agent - id=${agent.id}, name=${agent.name}`,
    );
    ...
    this.logger.log(
      `[PROCESSOR] Date range - from=${fromDateStr}, to=${toDateStr}, lastRunAt=${alert.lastRunAt?.toISOString() || "never"}`,
    );
...
`;
    ...
    this.logger.log(
      `[PROCESSOR] SENDING PROMPT TO AGENT - agentId=${agent.id}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `[PROCESSOR] AGENT RESPONSE RECEIVED - type=${typeof response}`,
      );
      this.logger.log(
        `[PROCESSOR] RAW AGENT RESPONSE: ${JSON.stringify(response).substring(0, 2000)}...`,
      );
    } catch (error) {
      this.logger.error(
        `[PROCESSOR] AGENT CALL FAILED - error=${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
    ...
    if (articles.length > 0) {
      this.logger.log(
        `[PROCESSOR] ARTICLES FOUND - ${JSON.stringify(articles.map((a) => ({ url: a.url, title: a.title, locationCount: a.locations?.length || 0 })))}`,
      );
      ...
      this.logger.log(
        `[PROCESSOR] executeAlert COMPLETE - savedCount=${savedCount}`,
      );
      return { articlesProcessed: savedCount };
    } else {
      this.logger.warn(
        `[PROCESSOR] NO ARTICLES PARSED from agent response! Check the raw response above.`,
      );
    }
  ...
  private async getOrCreateOrchestratorAgent() {
    ...
    this.logger.log(
      `[PROCESSOR] Found ${existingAgents.length} existing agents`,
    );
    ...
    if (orchestrator) {
      this.logger.log(
        `[PROCESSOR] FOUND existing orchestrator agent - id=${orchestrator.id}, name=${orchestrator.name}`,
      );
      return orchestrator;
    }

    this.logger.log(
      `[PROCESSOR] No existing orchestrator found, CREATING new agent with config: name=${NEWS_ORCHESTRATOR_CONFIG.name}, model=${NEWS_ORCHESTRATOR_CONFIG.model}`,
    );
    ...
    this.logger.log(
      `[PROCESSOR] CREATED new orchestrator agent - id=${newAgent.id}, name=${newAgent.name}`,
    );
    return newAgent;
  }
  ...
  private parseAgentResponse(response: {
    ...
    this.logger.log(
      `[PROCESSOR] parseAgentResponse START - messageCount=${response.messages?.length || 0}`,
    );

    if (!response.messages || response.messages.length === 0) {
      this.logger.warn(
        `[PROCESSOR] parseAgentResponse NO MESSAGES in response!`,
      );
      return [];
    }
    ...
    for (let i = 0; i < response.messages.length; i++) {
      ...
      this.logger.log(
        `[PROCESSOR] parseAgentResponse MESSAGE ${i + 1}/${response.messages.length} - type="${messageType}"`,
      );

      if (messageType === "assistant_message" && message.assistant_message) {
        this.logger.log(
          `[PROCESSOR] parseAgentResponse ASSISTANT MESSAGE - length=${message.assistant_message.length}`,
        );
        this.logger.log(
          `[PROCESSOR] parseAgentResponse ASSISTANT CONTENT (first 500 chars): ${message.assistant_message.substring(0, 500)}`,
        );
        const parsed = this.extractJsonFromText(message.assistant_message);
        if (parsed) {
          const normalized = this.normalizeArticles(parsed);
          this.logger.log(
            `[PROCESSOR] parseAgentResponse EXTRACTED ${normalized.length} articles from assistant_message`,
          );
          articles.push(...normalized);
        } else {
          this.logger.log(
            `[PROCESSOR] parseAgentResponse NO JSON found in assistant_message`,
          );
        }
      }
      ...
      if (messageType === "tool_return_message" && message.tool_return) {
        this.logger.log(
          `[PROCESSOR] parseAgentResponse TOOL RETURN - length=${message.tool_return.length}`,
        );
        this.logger.log(
          `[PROCESSOR] parseAgentResponse TOOL RETURN CONTENT (first 500 chars): ${message.tool_return.substring(0, 500)}`,
        );
        const parsed = this.extractJsonFromText(message.tool_return);
        if (parsed) {
          const normalized = this.normalizeArticles(parsed);
          this.logger.log(
            `[PROCESSOR] parseAgentResponse EXTRACTED ${normalized.length} articles from tool_return`,
          );
          articles.push(...normalized);
        } else {
          this.logger.log(
            `[PROCESSOR] parseAgentResponse NO JSON found in tool_return`,
          );
        }
      }
      ...
      if (message.content && typeof message.content === "string") {
        this.logger.log(
          `[PROCESSOR] parseAgentResponse CONTENT FIELD - length=${message.content.length}`,
        );
        this.logger.log(
          `[PROCESSOR] parseAgentResponse CONTENT (first 500 chars): ${message.content.substring(0, 500)}`,
        );
        const parsed = this.extractJsonFromText(message.content);
        if (parsed) {
          const normalized = this.normalizeArticles(parsed);
          this.logger.log(
            `[PROCESSOR] parseAgentResponse EXTRACTED ${normalized.length} articles from content field`,
          );
          articles.push(...normalized);
        } else {
          this.logger.log(
            `[PROCESSOR] parseAgentResponse NO JSON found in content field`,
          );
        }
      }
    }
    ...
    this.logger.log(
      `[PROCESSOR] parseAgentResponse FINAL - totalArticles=${articles.length}`,
    );
    if (articles.length > 0) {
      this.logger.log(
        `[PROCESSOR] parseAgentResponse ARTICLE TITLES: ${articles.map((a) => a.title).join(", ")}`,
      );
    }
  ...
  private extractJsonFromText(text: string): unknown {
    this.logger.log(
      `[PROCESSOR] extractJsonFromText - textLength=${text.length}`,
    );
    ...
    if (jsonMatches) {
      ...
      try {
        ...
        this.logger.warn(
          `[PROCESSOR] extractJsonFromText - FAILED to parse array JSON: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
  ...
  private normalizeArticles(data: unknown): ProcessedArticle[] {
    this.logger.log(
      `[PROCESSOR] normalizeArticles - isArray=${Array.isArray(data)}`,
    );

    if (!Array.isArray(data)) {
      this.logger.warn(
        `[PROCESSOR] normalizeArticles - Data is NOT an array, returning empty`,
      );
      return [];
    }

    this.logger.log(
      `[PROCESSOR] normalizeArticles - Processing ${data.length} items`,
    );
    ...
    const validItems = data.filter((item): item is Record<string, unknown> => {
      ...
      if (!isValid) {
        this.logger.log(
          `[PROCESSOR] normalizeArticles - SKIPPING invalid item (missing url or title): ${JSON.stringify(item).substring(0, 200)}`,
        );
      }
      return isValid;
    });
    ...
    this.logger.log(
      `[PROCESSOR] normalizeArticles - ${validItems.length}/${data.length} items are valid articles`,
    );

    return validItems.map((item, idx) => {
      const locations = this.normalizeLocations(item.locations);
      this.logger.log(
        `[PROCESSOR] normalizeArticles - Article ${idx + 1}: url=${item.url as string}, title="${(item.title as string).substring(0, 50)}...", locations=${locations.length}`,
      );
  ...
  private normalizeLocations(
    ...
    if (!Array.isArray(locations)) {
      this.logger.log(
        `[PROCESSOR] normalizeLocations - locations is NOT an array (type=${typeof locations}), returning empty`,
      );
      return [];
    }

    this.logger.log(
      `[PROCESSOR] normalizeLocations - Processing ${locations.length} locations`,
    );
    ...
    const validLocs = locations
      .filter((loc): loc is Record<string, unknown> => {
        ...
        if (!isValid) {
          this.logger.log(
            `[PROCESSOR] normalizeLocations - SKIPPING invalid location: ${JSON.stringify(loc).substring(0, 100)}`,
          );
        }
        return isValid;
      })
    ...
    this.logger.log(
      `[PROCESSOR] normalizeLocations - ${validLocs.length}/${locations.length} valid locations: ${validLocs.map((l) => `"${l.mention}"`).join(", ")}`,
    );
